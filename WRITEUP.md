...
- **Per-question calibration.** Replace the single global cap with a calibrator that conditions on the item, so the layer can *rank* as well as deflate. The bar is concrete: recover AUROC toward the raw 0.582 while holding ECE near 0.10 — deflate without flattening. Temperature scaling (Guo et al., 2017) is the natural first move: it's monotone, so it preserves whatever ranking signal the raw confidence carries while fixing the scale; isotonic/histogram binning or a small learned calibrator over answer features could go further. This directly targets the AUROC regression, which is the most important negative result here.
